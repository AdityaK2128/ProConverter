...
struct CurrentConversionView: View {
    ...
    let units = ["Amperes", "Milliamperes"]
    ...
    func convertValue(_ inputValue: Double) -> Double {
        if currentUnitIndex == 0 && desiredUnitIndex == 1 {
            // Conversion from Amperes to Milliamperes
            return inputValue * 1000
        } else if currentUnitIndex == 1 && desiredUnitIndex == 0 {
            // Conversion from Milliamperes to Amperes
            return inputValue / 1000
        } else {
            return inputValue
        }
...
#Preview {
   CurrentConversionView(isPresented: .constant(false)) // Initialize with false or true based on your needs
}
